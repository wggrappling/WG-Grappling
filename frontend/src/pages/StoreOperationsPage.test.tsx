...
vi.mock('../services', () => ({ storeOperations: { products: vi.fn(), reviews: vi.fn(), orders: vi.fn(), order: vi.fn(), createProduct: vi.fn(), updateProduct: vi.fn(), uploadProductImage: vi.fn(), addStock: vi.fn(), findCustomer: vi.fn(), createOrder: vi.fn(), submitManualPayment: vi.fn(), approve: vi.fn(), cancel: vi.fn(), refund: vi.fn(), updateOrderStatus: vi.fn() } }));
...
describe('StoreOperationsPage', () => {
  ...
  it('shows internal orders only to ADMIN and keeps server balances visible', async () => {
    mocked.orders.mockResolvedValue([{ id: 12, subtotal: 100, total: 100, paid: 30, balance: 70, status: 'IN_PRODUCTION', createdAt: '2026-08-27', student: { enrollmentNumber: 'WG-41', person: { name: 'Ana' } }, items: [{ id: 1, productName: 'Short', color: 'Preto', size: 'M', madeToOrder: true, quantity: 1, unitPrice: 100, subtotal: 100 }], payments: [] }]);
    renderRole('ADMIN');
