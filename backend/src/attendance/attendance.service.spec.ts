...
describe('AttendanceService', () => {
  ...
  it('verifica duplicidade durante todo o dia, independentemente do horário', async () => {
    await service.createBatch({ classId: 2, attendanceDate: '2026-08-12T12:00:00Z', students: [{ studentId: 3, status: AttendanceStatus.PRESENT }] });
    expect(prisma.attendance.findMany).toHaveBeenCalledWith(expect.objectContaining({ where: expect.objectContaining({ attendanceDate: { gte: new Date('2026-08-12T00:00:00.000Z'), lt: new Date('2026-08-13T00:00:00.000Z') } }) }));
  });
